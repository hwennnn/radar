...
func TestMergeCompanyLogoDomainsUsesOfficialSeedWebsites(t *testing.T) {
	domains := mergeCompanyLogoDomains(nil, lite.DiscoverySeed{Candidates: []lite.DiscoveryCandidate{
...
func TestCompanyLogoDomainsCoverDiscoveryRegistryAndVerifiedCatalog(t *testing.T) {
	domains := loadCompanyLogoDomains("../../lite/discovery-seed.json")
	if len(domains) < 450 {
		t.Fatalf("logo registry unexpectedly small: %d domains", len(domains))
	}
	catalog, err := loadCatalogFile("../../lite/verified-sources.json")
	if err != nil {
		t.Fatal(err)
	}

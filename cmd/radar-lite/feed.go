...
type feedStore interface {
	ListPostings(context.Context) ([]lite.Posting, error)
	ListSourceStatuses(context.Context) ([]lite.SourceStatus, error)
}
...
func buildFeed(postings []lite.Posting, statuses []lite.SourceStatus, totalSources int, now time.Time) ([]feedJob, feedSummary) {
	...
	for _, posting := range postings {
		if !lite.EligibleAt(posting, now) {
			continue
		}
...
func feedTrack(posting lite.Posting) string {
...
func safeApplyURL(raw string) string {
	return lite.CanonicalApplyURL(raw)
}

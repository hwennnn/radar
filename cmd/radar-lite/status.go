...
func buildStatusResponse(operational lite.OperationalState, cfg dashboardConfig, health *healthState) statusResponse {
	sourceMetadata := make(map[string]lite.Source, len(cfg.BaseSources))
	...
	statusByID := make(map[string]lite.SourceStatus, len(operational.RoutineSourceStatus))

...
func TestHealthHandlerKeepsLivenessIndependentOfCycleFailure(t *testing.T) {
	state := &healthState{}
	state.recordCycle(lite.RunReport{}, lite.DiscoveryReport{}, lite.DeliveryReport{}, errors.New("cycle failed"))
...
func TestReadOnlyReadinessRefreshesDurableRuntimeOnEveryRequest(t *testing.T) {
	...
	state := &healthState{}
	state.recordReadOnly(&lite.RuntimeState{
		LastCycleState: "success", LastCycleFinished: &oldFinished,
		SourcesSucceeded: 81,
	})
	state.setRuntimeReader(operationalReaderFunc(func(context.Context) (lite.OperationalState, error) {
		return lite.OperationalState{Runtime: &lite.RuntimeState{
...
func TestReadinessFailsClosedWhenDurableRuntimeCannotBeRead(t *testing.T) {
	...
	state.setRuntimeReader(operationalReaderFunc(func(context.Context) (lite.OperationalState, error) {
		return lite.OperationalState{}, errors.New("database unavailable")
	}), true)
	response := httptest.NewRecorder()
	state.handler().ServeHTTP(response, httptest.NewRequest(http.MethodGet, "/readyz", nil))
...
type operationalReaderFunc func(context.Context) (lite.OperationalState, error)

func (f operationalReaderFunc) ReadOperationalState(ctx context.Context) (lite.OperationalState, error) {
	return f(ctx)
}

func TestCycleResultStatusDistinguishesManagedDegradation(t *testing.T) {
	if got := cycleResultStatus(lite.RunReport{}, lite.DiscoveryReport{}, lite.DeliveryReport{}, errors.New("database failed")); got != "failure" {
		t.Fatalf("fatal cycle status = %q", got)
	}
	if got := cycleResultStatus(lite.RunReport{SourcesFailed: 1}, lite.DiscoveryReport{}, lite.DeliveryReport{}, nil); got != "degraded" {
		t.Fatalf("partial source status = %q", got)
	}
	if got := cycleResultStatus(lite.RunReport{}, lite.DiscoveryReport{CandidatesFailed: 1}, lite.DeliveryReport{}, nil); got != "degraded" {
		t.Fatalf("discovery status = %q", got)
	}
	if got := cycleResultStatus(lite.RunReport{}, lite.DiscoveryReport{}, lite.DeliveryReport{Failed: 1}, nil); got != "degraded" {
		t.Fatalf("delivery status = %q", got)
	}
	if got := cycleResultStatus(lite.RunReport{SourcesSucceeded: 70}, lite.DiscoveryReport{}, lite.DeliveryReport{}, nil); got != "success" {
...
func TestHealthHandlerReadinessTracksLastCompletedCycle(t *testing.T) {
	...
	state.recordCycle(lite.RunReport{SourcesSucceeded: 48}, lite.DiscoveryReport{}, lite.DeliveryReport{}, nil)
	assertReadiness(t, state, http.StatusOK, readinessExpectation{ready: true, sourcesSucceeded: 48})

	state.recordCycle(lite.RunReport{SourcesSucceeded: 3}, lite.DiscoveryReport{}, lite.DeliveryReport{}, errors.New("database failed"))
...
func TestHealthHandlerReportsManagedFailuresAsDegradedAndReady(t *testing.T) {
	state := &healthState{}
	state.recordCycle(
		lite.RunReport{SourcesSucceeded: 45, SourcesFailed: 3},
		lite.DiscoveryReport{},
		lite.DeliveryReport{Failed: 2},
		nil,
	)
...
func TestHealthHandlerReportsDiscoveryOnlyFailureAsDegradedAndReady(t *testing.T) {
	state := &healthState{}
	state.recordCycle(
		lite.RunReport{SourcesSucceeded: 70},
...
func TestLogSenderRejectsInvalidPayload(t *testing.T) {
	err := (logSender{logger: slog.New(slog.NewTextHandler(&bytes.Buffer{}, nil))}).Send(
		context.Background(), lite.Delivery{Payload: json.RawMessage(`{`)},
	)
	if err == nil {
...
func TestOutboxSenderMapsPostingWithoutNetwork(t *testing.T) {
	...
	delivery := lite.Delivery{ID: 7, JobID: "job-7", Channel: "telegram", Recipient: "chat-7", Payload: payload}
...
func (f *fakeOutbox) Enqueue(_ context.Context, message notify.Message) error {
	f.calls++
	f.message = message
	return nil
}

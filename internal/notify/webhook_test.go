...
func TestWebhookDefaultClientBlocksPrivateTargets(t *testing.T) {
	...
	outbox := NewDiscordWebhookOutbox(server.URL, nil)
...
func TestDiscordWebhookOutboxDisablesMentions(t *testing.T) {
	var payload map[string]any
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
			t.Fatalf("decode payload: %v", err)
	...
	outbox := NewDiscordWebhookOutbox(server.URL, server.Client())
	if err := outbox.Enqueue(context.Background(), Message{Body: "@everyone Radar match"}); err != nil {
		t.Fatalf("Enqueue() error = %v", err)
	}
	...
	if !ok || len(parse) != 0 {
		t.Fatalf("allowed_mentions.parse = %#v, want empty array", allowed["parse"])
	}
}
...
func TestSlackWebhookOutboxReturnsStatusError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		http.Error(w, "webhook unavailable", http.StatusBadGateway)
	}))
	defer server.Close()

	outbox := NewSlackWebhookOutbox(server.URL, server.Client())
	err := outbox.Enqueue(context.Background(), Message{Body: "Radar match"})
	if err == nil || !strings.Contains(err.Error(), "slack webhook failed") || !strings.Contains(err.Error(), "502") {
		t.Fatalf("Enqueue() error = %v, want slack status error", err)
	}
}

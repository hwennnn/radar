...
func NewDiscordWebhookOutbox(webhookURL string, client *http.Client) *WebhookOutbox {
	return &WebhookOutbox{
		provider: "discord",
		endpoint: strings.TrimSpace(webhookURL),
		client:   defaultHTTPClient(client),
		payload: func(msg Message) (any, error) {
			return map[string]any{
				"content": renderWebhookText(msg, 1900),
...
func NewSlackWebhookOutbox(webhookURL string, client *http.Client) *WebhookOutbox {
	return &WebhookOutbox{
		provider: "slack",
		endpoint: strings.TrimSpace(webhookURL),
		client:   defaultHTTPClient(client),
		payload: func(msg Message) (any, error) {
			return map[string]any{"text": renderWebhookText(msg, 3000)}, nil
		},
	}
}

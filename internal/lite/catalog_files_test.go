...
func TestRealCatalogFilesKeepRoutineAndDiscoverySeparate(t *testing.T) {
	catalogFile := openLiteDataFile(t, "verified-sources.json")
	...
	seedFile := openLiteDataFile(t, "discovery-seed.json")
...
func openLiteDataFile(t *testing.T, name string) *os.File {
	t.Helper()
	file, err := os.Open(filepath.Join("..", "..", "lite", name))
	if err != nil {

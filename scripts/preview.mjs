...
function startPreview() {
  ...
  server.on('error', error => {
    if (error && error.code === 'EADDRINUSE') {
      console.error(`radar preview: ${host}:${port} is already in use`);
    } else {
      console.error('radar preview failed:', error);
    }
    process.exitCode = 1;
  });
  server.listen(port, host, () => {
    console.log(`radar preview: http://${host}:${port}`);
    console.log(`radar preview: API upstream ${upstream}`);
  });
